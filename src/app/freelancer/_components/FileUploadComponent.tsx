...
//         chunkFormData.append("chunkNumber", (index + 1).toString());
...
//       const finalResponse = await completeResponse.json();

//       // Add to uploaded files list
//       const fileUrl = finalResponse.fileUrl || `${process.env.NEXT_PUBLIC_S3_BUCKET_URL}/${key}`;
//       setUploadedFiles(prev => [...prev, { name: file.name, url: fileUrl }]);

//       if (onUploadComplete) {
//         onUploadComplete({ name: file.name, url: fileUrl });
//       }
...
//                 <div key={index} className="flex flex-col p-3 bg-white rounded-lg shadow-sm border border-gray-100">
...
//                   {uploadProgress[file.name] === 100 && !uploadedFiles[index]?.url && (
