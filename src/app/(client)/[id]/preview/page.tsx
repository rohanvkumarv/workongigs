...
const AudioPreview = ({ url, filename, isPaid }) => {
  ...
  useEffect(() => {
    if (audioRef.current) {
      const handleTimeUpdate = () => {
        ...
        // Stop playback if preview limit is reached and not paid
  ...
  const handlePlayPause = () => {
    if (!isPaid && hasReachedPreviewLimit) {
      return; // Prevent playing if preview limit reached
    }
    ...
    if (isPlaying) {
      ...
      if (!isPaid && currentTime >= PREVIEW_DURATION) {
        audioRef.current.currentTime = 0; // Reset to start for preview
      }
      audioRef.current?.play();
    }
    setIsPlaying(!isPlaying);
  ...
  const handleProgressClick = (e) => {
    if (!isPaid) return; // Disable seeking for unpaid preview
...
const PaymentDownloadSection = ({ 
  ...
  const totalProjectPrice = projectData.instances.reduce((sum, instance) => 
    instance.paymentStatus === "Not Paid" ? sum + instance.cost : sum, 0);

  return (
    <div className="space-y-4 md:space-y-6">
      {/* Main Payment/Download Box */}
      <div className="bg-white rounded-xl md:rounded-2xl p-4 md:p-6 shadow-lg border border-gray-100">
        <AnimatePresence mode="wait">
          {currentInstance.paymentStatus === "Not Paid" ? (
            <motion.div
              key="payment-options"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              ...
              {/* Payment Buttons */}
              <div className="space-y-3">
                {/* Individual Instance Payment */}
                <motion.button
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => handlePayment(currentInstance.cost, currentInstance.id, false)}
                  className="w-full py-3 bg-black text-white rounded-xl font-medium
                           flex items-center justify-center gap-2"
                >
                  <CreditCard className="w-5 h-5" />
                ...
                {/* All Instances Payment */}
                {totalProjectPrice > 0 && (
                  <motion.button
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    onClick={() => handlePayment(totalProjectPrice, null, true)}
                  ...
                  >
                    <Package className="w-5 h-5" />
                    Pay for All Instances (₹{totalProjectPrice})
                  </motion.button>
                )}
              </div>
            </motion.div>
          ...
          if (result.success) {
            ...
            if (payAllInstances) {
              ...
              updatedInstances.forEach(instance => {
                if (instance.paymentStatus === "Not Paid") {
                  instance.paymentStatus = "Paid";
                }
              });
            } else {
              ...
              const index = updatedInstances.findIndex(i => i.id === instanceId);
              if (index !== -1) {
                updatedInstances[index].paymentStatus = "Paid";
              }
            }
