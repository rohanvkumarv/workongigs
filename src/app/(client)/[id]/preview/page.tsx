...
const PreviewPayment = () => {
  ...
  const router = useRouter();
  const clientId = params.id;
  // Changed from urlDeliveryName to urlDeliveryId
  const urlDeliveryId = searchParams.get('delivery');
  ...
  // Update selectedDelivery when URL changes - now using delivery ID
  useEffect(() => {
    if (clientData && urlDeliveryId) {
      const deliveryIndex = clientData.deliveries.findIndex(
        d => d.id === urlDeliveryId
      );
      if (deliveryIndex !== -1) {
        setSelectedDelivery(deliveryIndex);
        setShowOtherDeliveries(false);
  ...
  useEffect(() => {
    const fetchClientData = async () => {
      try {
        ...
        // Updated to use delivery ID instead of name
        if (urlDeliveryId) {
          const deliveryIndex = client.deliveries.findIndex(
            d => d.id === urlDeliveryId
          );
  ...
  // Handle delivery change - updated to use delivery ID in URL
  const handleDeliveryChange = (idx) => {
    const newDeliveryId = clientData.deliveries[idx].id;
    const newUrl = `/${clientId}/preview?delivery=${newDeliveryId}`;
    window.history.replaceState({ ...window.history.state, as: newUrl, url: newUrl }, '', newUrl);
    setSelectedDelivery(idx);
    setCurrentSlide(0);
  };
  ...
  // Payment handler - updated to use delivery ID in URL update
  const handlePayment = async (amount, deliveryId = null, payAllDeliveries = false) => {
    try {
      setPaymentStatus({
        show: true,
        ...
        handler: async (res) => {
          ...
          try {
            ...
            if (result.success) {
              ...
              // Update URL to reflect current delivery using delivery ID
              const currentDelivery = updatedDeliveries[updatedDeliveryIndex];
              const newUrl = `/${clientId}/preview?delivery=${currentDelivery.id}`;
              window.history.replaceState({ ...window.history.state, as: newUrl, url: newUrl }, '', newUrl);
